...
fundic <- function(mm){
  ...
  for(i in 1:I){
    for(j in 1:(I+1-i)){
      ddic <- ddic  -2*dnorm(log(data[i,j]), logulti[mm, i] + log(fun1(j, exp(logwtheta[mm,1]),
                                                            exp(logwtheta[mm,2]))),
            sigma2[mm]^0.5, log = T)       
    }
  }
...
dbar <- sapply(1001:10000, fundic)
dbar <- mean(dbar)
dbar2 <- 0
for(i in 1:I){
  for(j in 1:(I+1-i)){
    dbar2 <- dbar2-2*dnorm(log(data[i,j]), mean(logulti[1001:10000, i]) + log(fun1(j, exp(mean(logwtheta[1001:10000,1])),
                                                                                  exp(mean(logwtheta[1001:10000,2])))),
                          mean(sigma2[1001:10000])^0.5, log = T)      
  }
}
pd <- dbar-dbar2
dic <- dbar2+2*pd
