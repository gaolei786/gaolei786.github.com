...
I Sb Sg D N phi chi_b chi_g nu 
;
...
phi_g sigma_A theta1 theta2 delta_X  d0 d1 d2 rho_c
kappa kappa_b kappa_g gamma zeta pi_b erow
...
Iss1 Sbss1 Sgss1 Dss1 Nss1 phiss1 chi_bss1 chi_gss1 nuss1 value1ss1 value2ss1
...
Iss2 Sbss2 Sgss2 Dss2 Nss2 phiss2 chi_bss2 chi_gss2 nuss2 value1ss2 value2ss2
;
...
d2 = 4.7958e-08; %s
rho_c = 2; %s
%kappa = 0.337388881860543;  %s
kappa = 0.3374;
gamma = 0.974;  %s
%zeta = 0.002284982884902;  %s
zeta = 0.0023;
%erow = 1.372382641894985;
erow = 1.3724;
%pi_b = 0.702469212147520; %before
pi_b = 0.7025;
kappa_b = 1; %before
kappa_g = 1; %before


%tau_ess1 = 0.006950972307235;
tau_ess1 = 0.007;
Ass1 = 1;
tau_bss1 = 0;
tau_gss1 = 0;
...
value1ss1 = varho/((1-alpha_b)*(p_bss1 - (1-tau_zss1)*p_bss1*theta1*(muss1^theta2) - tau_ess1*(1-muss1))); %B.21
value2ss1 = varho/((1-alpha_g)*p_gss1*(1+tau_gtss1)); %B.24
Lbss1 = (Ybss1/value1ss1)^(1/2);
Lgss1 = (Ygss1/value2ss1)^(1/2);
...
Qbss1 = p_bss1; %B.26
Qgss1 = p_gss1; %B.26
Rbss1 = (alpha_b*(Ybss1/Kbss1) * (p_bss1 - (1-tau_zss1)*p_bss1*theta1*muss1^(theta2)-tau_ess1*(1-muss1 ) ) + (1-delta_b)*Qbss1 ) / Qbss1;%B.23
Rgss1 = alpha_g*(1+tau_gtss1)*Ygss1/Kgss1 + (1-delta_g); %B.25
...
chi_bss1 =  Mss1*(1-gamma + gamma* phiss1)*(Rbss1- (1+tau_bss1)*Rss1) ; %B.5
chi_gss1 =  Mss1*(1-gamma + gamma* phiss1)*(Rgss1- (1+tau_gss1)*Rss1) ; %B.6
nuss1 =  Mss1*(1-gamma + gamma* phiss1) * Rss1 ; %B.8
...
tau_gtss2 = 0.05;
tau_zss2 = 0;
Ygss2 =  0.581285270715504;
Ybss2 =  0.842830564647928 ;
Cgss2 = 0.435440997344744;
Cbss2 = 0.630029889799517 ;
...
value1ss2 = varho/((1-alpha_b)*(p_bss2 - (1-tau_zss2)*p_bss2*theta1*(muss2^theta2) - tau_ess2*(1-muss2))); %B.21
value2ss2 = varho/((1-alpha_g)*p_gss2*(1+tau_gtss2)); %B.24
Lbss2 = (Ybss2/value1ss2)^(1/2);
Lgss2 = (Ygss2/value2ss2)^(1/2);
...
Qbss2 = p_bss2; %B.26
Qgss2 = p_gss2; %B.26
Rbss2 = (alpha_b*(Ybss2/Kbss2) * (p_bss2 - (1-tau_zss2)*p_bss2*theta1*muss2^(theta2)-tau_ess2*(1-muss2 ) ) + (1-delta_b)*Qbss2 ) / Qbss2;%B.23
Rgss2 = alpha_g*(1+tau_gtss2)*Ygss2/Kgss2 + (1-delta_g); %B.25
...
chi_bss2 =  Mss2*(1-gamma + gamma* phiss2)*(Rbss2- (1+tau_bss2)*Rss2) ; %B.5
chi_gss2 =  Mss2*(1-gamma + gamma* phiss2)*(Rgss2- (1+tau_gss2)*Rss2) ; %B.6
nuss2 =  Mss2*(1-gamma + gamma* phiss2) * Rss2 ; %B.8
...
moni1.Y          = (Y/Y(1)-1)*100;
moni1.Yb          = (Yb/Yb(1)-1)*100;
moni1.Yg          = (Yg/Yg(1)-1)*100;
moni1.Kb          = (Kb/Kb(1)-1)*100;
moni1.Kg          = (Kg/Kg(1)-1)*100;
moni1.Qb          = (Qb/Qb(1)-1)*100;
moni1.Qg          = (Qg/Qg(1)-1)*100;
moni1.I         = (I/I(1)-1)*100;
moni1.C         = (C/C(1)-1)*100;
moni1.e_b        = (e_b/e_b(1)-1)*100;
moni1.N          = (N/N(1)-1)*100;
moni1.L          = (L/L(1)-1)*100;
moni1.p_g = (p_g/p_g(1)-1)*100;
moni1.p_b = (p_b/p_b(1)-1)*100;